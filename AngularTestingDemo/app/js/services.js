angular.module('myApp.services', ['ngMockE2E'])
    .value('version', '0.1')
    .run(['$httpBackend', function ($httpBackend) {
        ...
        $httpBackend.whenGET('/categories').respond(data.categories);
        $httpBackend.whenGET(/\/categories\/.+/).respond(function (method, url) {
            var regexp = new RegExp(/\/categories\/(.+)/);
            var match = regexp.exec(url);
            var id = match[1];
            return [200, angular.toJson(data.categories[id])];
        });
    } ]);
